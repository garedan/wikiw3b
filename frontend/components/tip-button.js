...
export default function TipButton({ ethereum, index }) 
{
    ...
    const submitTip = async (e) => {
        ...
        try {
            ...
            const postsContract = new ethers.Contract(abiAddPostAddress, AddPost.abi, signer);
            const tipTxn = await postsContract.tip(index, { value: ethers.utils.parseEther("0.01") })
    ...
    return <SecondaryButton onClick={submitTip} disabled={mining}>
                {mining ? 'Enviando...' : 'Propina de 0.01 eth!'}
            </SecondaryButton>
}